...
//					YUVMatrix[row][pixel] += RGBToYUVFilter[row][k] * RGBMatrix[k][pixel];
	...
	public static BufferedImage doImageprocessing(String args[])  {
	...
	private static byte[] quantizeImage(byte[] rgbBytes, int q) {
		...
		// generate new array of reduced vals
		Quantizer.generateReducedValsArray(q);
		
		for (int i = 0; i < rgbBytes.length; i++) {
			int byteVal = (int)rgbBytes[i] & 0x000000FF;
			//double newbyteval = Quantizer.midRiserQuantizer(byteVal, q, 256);
			rgbBytes[i] = (byte)(Quantizer.myQuantizer(byteVal, q) & 0x000000ff);
		...
		System.out.println(Arrays.toString(Quantizer.reducedVals));
		return rgbBytes;
